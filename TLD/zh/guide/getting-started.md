...
> [!WARNING]
> 注意：本文档及其以下所有文档主要针对 **TLD**（Rust 版本）
> 
> 如果您使用的是 **TTHSD Golang** 版本，请注意：
> - [TTHSD Golang](https://github.com/TaiLerDownloader/TTHighSpeedDownloader) 已停止开发
> - 建议迁移到 TLD 以获得更好的性能和功能
> - 调用方式与 TLD 大致相同，可以直接替换动态库
> - [TTHSD Golang](https://github.com/TaiLerDownloader/TTHighSpeedDownloader) 的官方发布最新版（ 0.5.0 版本 ）有 **Bug** ，不建议使用。
>
> 新版文档未完全更新，请等待更新。
> [TLD](https://github.com/TaiLerDownloader/TaiLerDownloader) 现在还处于实验性阶段，但其核心的基本功能均可使用， Android , IOS 和 HarmonyOS 的编译版本 未经过大规模测试，可以的话请帮忙测试，谢谢！

## 简介

TLD Core 是一个高性能的多线程下载器内核，原生支持 **7 种下载协议**（HTTP/HTTPS、HTTP/3、FTP、SFTP、BitTorrent/Magnet、ED2K、Metalink）。TLD 采用 **Rust 语言**编写，核心逻辑编译为动态链接库 (DLL/SO/DYLIB)，可供 Python、C++、C#、Java/Kotlin、TypeScript、Rust、Go、Godot 等 **8 种语言**调用。

相比 [TTHSD Golang](https://github.com/TaiLerDownloader/TTHighSpeedDownloader) 版本，TLD 具有更高的性能、更低的内存占用和更稳定的运行表现。

本文档将指导您如何在 Python 项目中快速集成和使用 TLD。
...
- **操作系统**: Windows (x64), Linux (Ubuntu 22.04+), macOS, Android, IOS, HarmonyOS
...
> [!TIP]
> 您可以在 [TLD GitHub Releases](https://github.com/TaiLerDownloader/TaiLerDownloader/releases/latest) 页面下载最新版本的内核文件。

### 3. 获取 Python 接口封装

为了简化调用，官方提供了封装好的 Python 接口文件 ***tld_interface.py***。

1. 下载 ***tld_interface.py***。
2. 将 **内核文件** 和 ***tld_interface.py*** 放置在您的 Python 项目根目录下。
...
- 访问 [TLD GitHub](https://github.com/TaiLerDownloader/TaiLerDownloader) 获取源代码和更多信息。
