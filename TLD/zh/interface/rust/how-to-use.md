...
> [!TIP]
> 本文档介绍的是 **TLD** 的 Rust 封装 Crate。
> 源码位于 [***bindings/rust/***](https://github.com/TaiLerDownloader/TaiLerDownloader/tree/main/bindings/rust)
...
用户代码 (safe Rust)
    │
    ▼
TaiLerDownloader          ← downloader.rs (safe API + mpsc channel)
    │
    ▼
...
```rust
use TaiLerDownloader::{TaiLerDownloader, DownloadOptions};

#[tokio::main]
async fn main() {
    let dl = TaiLerDownloader::load(None)
        .expect("加载动态库失败");
...
```toml
[dependencies]
TaiLerDownloader = { path = "../bindings/rust" }
