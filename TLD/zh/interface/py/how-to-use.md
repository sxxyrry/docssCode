...
> [!TIP]
> 本文档介绍的是 **TLD**（Rust 版本）的 Python 接口。
> 
...
本文档介绍 ***tld_interface.py*** 模块，该模块为 **TT高速下载器**（***TaiLerDownloader.dll***/***.so***/***.dylib***）提供 Python 封装，支持多任务下载的创建、控制与进度回调。
...
2. TT高速下载器动态库文件：
   ...
   - macOS 平台：***libTaiLerDownloader.dylib***
   - Linux 平台：***libTaiLerDownloader.so***

### 0.2 安装步骤

1. 将动态库文件放置在项目目录中
2. 将 ***tld_interface.py*** 复制到项目目录

---

## 1. 模块概述

- **核心类**：***TaiLerDownloader***
...
### 2.1 初始化
```python
downloader = TaiLerDownloader(dll_path: str | pathlib.Path | None = None)
```
- **参数** ***dll_path***：动态库路径。若为 ***None***，根据操作系统自动猜测默认文件名（Windows: ***TaiLerDownloader.dll***，macOS: ***libTaiLerDownloader.dylib***，Linux: ***libTaiLerDownloader.so***），路径为当前工作目录。
...
### 4.1 基本用法（立即启动）
```python
from tld_interface import TaiLerDownloader
...
def on_progress(event, msg):
    ...
    # 剩下的处理可以去查看 示例/TLD Python 接口封装 的 Callback 示例

with TaiLerDownloader() as dl:
...
- 这些引用会随着 ***TaiLerDownloader*** 实例一起被 Python 垃圾回收器自动清理
- 无需手动干预或调用额外的清理方法
...
- 输出到控制台
- 写入文件 ***TLDPyInter.log***（位于可执行文件目录或当前目录）
- 日志级别：***INFO***
...
5. **多线程环境**：不同线程使用不同的 ***TaiLerDownloader*** 实例是安全的。同一实例的方法（除回调注册外）在 Python 层面未加锁，但 DLL 内部状态由自身管理。

---

## 8. TLD 的优势

相比 TTHSD Golang 版本，使用 TLD 的 Python 接口可以获得以下优势：
...
**Q: 需要并发下载多个任务组怎么办？**  
A: 创建多个 ***TaiLerDownloader*** 实例，每个管理一组任务。它们独立运行。或配合 ***threading*** 模块执行多次 开始下载
...
如有问题，请参考 [TLD GitHub](https://github.com/TaiLerDownloader/TaiLerDownloader) 或提交 issue。
