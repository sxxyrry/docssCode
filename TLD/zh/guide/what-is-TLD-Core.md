...
**TLD 核心**（TaiLerDownloader Core）是一个高性能、跨平台、多语言可调用的下载引擎内核，可为外部项目提供强大的下载能力支持，使开发者能够在自己的应用中轻松集成专业级的文件下载功能。
...
> [!TIP]
> 本文档介绍的是 **TLD**（Rust 版本），它是 [TTHSD Golang](https://github.com/TaiLerDownloader/TTHighSpeedDownloader) 的 Rust 重写版本。
> 
> TLD 的调用方式与 TTHSD Golang 相同，但性能更好。
> 
> 注：[TTHSD Golang](https://github.com/TaiLerDownloader/TTHighSpeedDownloader) 已停止开发，建议新项目使用 TLD。

## 核心概念

### 1. 高性能下载引擎
TLD 核心采用 **Rust 语言**编写，利用 Rust 的异步特性和零成本抽象实现了真正的多线程下载。它能够：
- 自动分割大文件为多个小块并行下载
- 智能调整线程数量以最大化带宽利用率
...
### 3. 多语言调用接口
TLD 核心通过标准的 C ABI（应用程序二进制接口）暴露功能，支持：
- **Python**: 通过 ***tld_interface.py*** 封装调用
- **C/C++**: 通过 ***TaiLerDownloader.h*** / ***TaiLerDownloader.hpp*** 封装调用
- **C#/.NET**: 通过 ***TaiLerDownloader.cs*** P/Invoke 封装调用
...
### 4. 零性能损失的 Python 封装
TLD 提供了官方 接口封装 （例如***tld_interface.py***），其特点包括：
...
TLD 核心解决了所有这些痛点，让您能够：
```python
# 只需几行代码即可集成专业级下载功能
from tld_interface import TLDownloader

downloader = TLDownloader()
...
TLD 内置了协议路由工厂，根据 URL 前缀自动选择最优的下载器，原生支持 **7 种下载协议**：
...
1. 下载对应平台的动态链接库
2. 将库文件放入项目目录
3. ( Python ) 复制 ***tld_interface.py*** 到项目
4. 按照文档调用 API
...
无论您是在开发桌面应用、后端服务还是移动应用，TLD 核心都能为您提供可靠、高效的下载功能，让您专注于核心业务逻辑的开发。

---

**下一步**:
- [快速开始](/zh/guide/getting-started) - 立即开始使用 TLD
