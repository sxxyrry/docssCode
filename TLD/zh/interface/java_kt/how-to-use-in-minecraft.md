...
> [!TIP]
> TLD 可作为 Minecraft Mod/Plugin 的下载加速引擎，
...
将 TLD 动态库打包到 JAR 的 ***resources/native/*** 目录：

```
src/main/resources/native/
├── windows-x86_64/TaiLerDownloader.dll
├── linux-x86_64/libTaiLerDownloader.so
└── macos-aarch64/libTaiLerDownloader.dylib
```
...
```kotlin
import com.TaiLerDownloader.TaiLerDownloader

class DownloadManager {
    private val downloader = TaiLerDownloader()
...
- **线程安全**：TLD 内部使用 Tokio 异步运行时，回调会在 Rust 的异步线程中触发。如果需要更新 GUI，请在回调中使用 ***SwingUtilities.invokeLater()*** 或 ***Platform.runLater()*** 切换到 UI 线程
