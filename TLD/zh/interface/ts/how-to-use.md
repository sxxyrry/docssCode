...
> [!TIP]
> 本文档介绍的是 **TLD**（Rust 版本）的 TypeScript 接口。
> 
...
本文档介绍 ***TaiLerDownloader*** TypeScript 类，该类为 **TT高速下载器**（***TaiLerDownloader.dll***/***TaiLerDownloader.so***/***TaiLerDownloader.dylib***）提供 Node.js 封装，支持多任务下载的创建、控制与进度回调。该封装以 npm 包 ***TaiLerDownloader*** 形式发布。
...
# 或
yarn add TaiLerDownloader
```

***TaiLerDownloader*** 已内置所需的 [Koffi](https://koffi.dev) 依赖（用于调用动态库），并提供了完整的 TypeScript 类型定义，因此无需单独安装其他 FFI 包。

### 0.3 动态库文件
将 TT 高速下载器动态库放置在项目可访问的路径：
- Windows 平台：***TaiLerDownloader.dll***
- macOS 平台：***TaiLerDownloader.dylib***
- Linux 平台：***TaiLerDownloader.so***

> [!TIP]
> 您可以在 [TLD Releases](https://github.com/TaiLerDownloader/TaiLerDownloader/releases/latest) 页面下载最新版本的动态库文件。

---

## 1. 模块概述

- **核心类**：***TaiLerDownloader***（默认导出）
...
```typescript
import { quickDownload } from "TaiLerDownloader";
...
const downloader = new TaiLerDownloader(); // 自动搜索动态库
...
const downloader = new TaiLerDownloader({ dllPath: './custom/path/libTaiLerDownloader.so' });
const id = downloader.getDownloader(
...
### 5.3 使用内置事件日志器
```typescript
import { TaiLerDownloader, EventLogger } from 'TaiLerDownloader';

const logger = new EventLogger(); // 内置的控制台打印器
const downloader = new TaiLerDownloader();
...
相比 TTHSD Golang 版本，使用 TLD 的 TypeScript 接口可以获得以下优势：
...
**Q: 需要并发下载多个任务组怎么办？**
A: 创建多个 ***TaiLerDownloader*** 实例，每个管理一组任务。它们独立运行。或配合 ***worker_threads*** 模块执行多次开始下载。

**Q: TLD 与 TTHSD Golang 有什么区别？**  
A: TLD 是 TTHSD Golang 的 Rust 重写版本，具有更高的性能、更低的内存占用和更稳定的运行表现。API 接口完全兼容，只需替换动态库即可迁移。

**Q: 我可以从 TTHSD Golang 迁移到 TLD 吗？**  
A: 是的，API 接口完全兼容，只需将动态库文件从 TTHighSpeedDownloader.dll/so/dylib 替换为 TaiLerDownloader.dll/so/dylib 即可，代码无需修改。

**Q: 安装 ***TaiLerDownloader*** 后还需要单独安装 ***koffi*** 吗？**
A: 不需要。***TaiLerDownloader*** 已将其作为依赖包含，您只需安装这一个包即可。
...
如有问题，请参考 [TLD GitHub](https://github.com/TaiLerDownloader/TaiLerDownloader) 、 [TaiLerDownloader-interface (TypeScript) GitHub ](https://github.com/sxxyrry/TaiLerDownloader-interface-ts) 或 [API 文档](/zh/api/API-overview.md)。
