...
class MyApp : Application() {
    override fun onCreate() {
        super.onCreate()
        TLDLibraryJNI.load()  // System.loadLibrary("TLD")
    }
}
```
...
```kotlin
val id = TLDLibraryJNI.startDownload(
    tasksJson = """[{
    ...
    chunkSizeMB = 10,
    useCallbackUrl = true,
    callbackUrl = "ws://localhost:8080/TaiLerDownloader",
    useSocket = false,
    isMultiple = false
)
...
- **Android**: 将 ***libTaiLerDownloader.so*** 放到 ***src/main/jniLibs/<\abi>/*** */ 目录
  - ***arm64-v8a/libTaiLerDownloader.so***
  - ***armeabi-v7a/libTaiLerDownloader.so***
...
| 回调 | 函数指针 | 远程回调 URL |
| 接口类 | TLDLibraryJNA | TLDLibraryJNI` |
| 封装类 | TaiLerDownloader | TaiLerDownloaderAndroid（可选） |
