...
> [!TIP]
> 本文档介绍的是 **TLD**（Rust 版本）的 C / C++ / C# 接口。
> 源码位于 [***bindings/c/***](https://github.com/TaiLerDownloader/TaiLerDownloader/tree/main/bindings/c)
...
| 文件 | 说明 |
|------|------|
| ***TaiLerDownloader.h*** | 标准 C 头文件——声明所有 C ABI 导出函数及回调类型 |
| ***TaiLerDownloader.hpp*** | C++ header-only 封装类——RAII 持有库句柄，***std::function*** 回调 |
| ***csharp/TaiLerDownloader.cs*** | C# P/Invoke 封装——***async/await*** 事件流，支持 WPF / AvaloniaUI / Unity |

---

## C 接口 (***TaiLerDownloader.h***)

### 回调类型

```c
typedef void (*TLD_Callback)(const char* event_json, const char* data_json);
```
...
int start_download(
    ...
    TLD_Callback  callback,            // 回调函数指针（可为 NULL）
...
```cpp
#include "TaiLerDownloader.hpp"

int main() {
    TaiLerDownloader dl;
    dl.load();  // 自动搜索 TaiLerDownloader.dll / libTaiLerDownloader.so / libTaiLerDownloader.dylib
...
await using var dl = new TaiLerDownloader();
